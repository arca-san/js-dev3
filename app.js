...
function addBtns(btns){
  for (let item in btns){
    let btnItem = document.createElement("p");
    btnItem.classList.add("btn-item");
    btnItem.setAttribute("data-id", btns[item].name)
    btnItem.innerHTML = `<button>${btns[item].name}</button>`
    btnItem.addEventListener('click', buttonClick);
    btnContainerDOM.append(btnItem);
  }
}
